 ...
 *                                               this needs no translation at
 *                                               all, just to not be broken by
 *                                               anything above it
 *
 * Ported from Personal CRM's tools/test-harness.php (itself the pattern this
 * whole build is ported from) — same mechanism, same two rules, adapted only
 * for Keepsake's schema and the CLI-only override's global name.
 */
...
function harness_translate_statement(string $statement): array
{
    ...
    if ($deferredPrimary !== null && strcasecmp($deferredPrimary, (string) $autoCol) !== 0) {
         ...
         *
         * Personal CRM's person_tag_map (person_id, tag_id) and
 ...
 * every ON DELETE CASCADE/SET NULL below a no-op and let a test claim a
 * cascade works when nothing was ever enforced — photos alone cascades from
 * year_projects and sets-null from event_groups, and the book_layout tree
 * cascades three levels deep from a single year_projects delete.
 */
function harness_pdo(?string $schemaPath = null): PDO
{
    ...
    $GLOBALS['keepsake_pdo_override'] = $pdo;
    return $pdo;
}
